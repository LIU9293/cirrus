// The authoring SPEC handed to the Terr Developer Agent. This is the "规范"
...
// talks to the host exclusively through the TerrUI SDK.
...
\`@/terrui\` SDK. A miniapp is "agent-native" because its buttons can call back to
...
- Do NOT write main.tsx, index.css, or terrui.tsx — those are provided by the host.

## What is available
- React 19 (hooks). Import from "react".
- Tailwind utility classes (v4) — just use className. No config needed.
- The SDK: \`import { useTerrState, useTerr, useAgentAction, useAgentDataSource, useAutoResize } from '@/terrui'\`.
- Nothing else. No fetch, no axios, no other npm packages, no <script>, no CDN links.

## The SDK (@/terrui)
- \`useTerrState<T>(): T\` — the host-owned, persisted state model. Re-renders on host pushes.
- \`useTerr(): { setState(patch), action(id, payload), openLink(href) }\`
...
1. The UI renders from \`useTerrState()\`.
...
- Only import "react" and "@/terrui" (plus your own relative files). Nothing else.
...
spec above: React + Tailwind + TerrUI only.
