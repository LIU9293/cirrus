...
export const MiniappCanvas = forwardRef<MiniappCanvasHandle, Props>(
    ...
    return (
      <div className="flex h-full w-full flex-col">
            ...
            <div className="flex min-w-0 items-center gap-2.5">
              ...
              <div className="min-w-0">
                <div className="truncate text-sm font-semibold">
                  {miniapp?.manifest?.name ?? "Canvas"}
                </div>
                <div className="truncate text-xs text-muted-foreground">
                  {miniapp?.manifest?.description ??
        ...
        <div className="relative min-h-0 flex-1 bg-muted/30">
          {miniapp?.status === "building" && (
            <Overlay
              icon={<Loader2 className="size-5 animate-spin" />}
              text="Building the miniapp…"
            />
          )}
          {miniapp?.status === "error" && !miniapp.html && (
            <Overlay
              icon={<AlertTriangle className="size-5 text-destructive" />}
              text={miniapp.buildError ?? "Build failed."}
              mono
            />
          )}
          {!miniapp?.html &&
            miniapp?.status !== "building" &&
            miniapp?.status !== "error" && <EmptyCanvas />}
          {srcDoc && (
            <>
              <iframe
                ref={iframeRef}
              ...
              />
              {selectedElement && (
                <SelectionOverlay selection={selectedElement} />
              )}
            </>
          )}
        </div>
        ...
        {miniapp?.status === "error" && miniapp.html && (
          <div className="max-h-32 overflow-auto border-t border-destructive/40 bg-destructive/5 px-4 py-2 text-xs text-destructive">
            <div className="mb-1 font-semibold">
              Last build failed (showing previous version)
            </div>
      ...
      <div className="relative flex max-w-sm flex-col items-center gap-4 text-center">
        ...
        <div>
          ...
          <p className="mt-1.5 text-[13px] leading-relaxed text-muted-foreground">
            Describe the app you want in the build chat — I&apos;ll sketch it
            right here.
          </p>
        </div>
      </div>
      ...
      <div className="flex max-w-md flex-col items-center gap-3 text-center">
        ...
        <p
          className={cn(
            ...
            mono && "whitespace-pre-wrap text-left font-mono text-xs",
          )}
        >
          {text}
        </p>
